#!/usr/bin/env sh
# SessionStart hook: injects the full meini-style ruleset when the user has
# opted in by creating $CLAUDE_CONFIG_DIR/.meini-style-always (default ~/.claude).
...
flag_path="$claude_dir/.meini-style-always"
...
skill_path="$script_dir/../skills/meini-style/SKILL.md"
[ -f "$skill_path" ] || exit 0
...
printf 'MEINI STYLE ACTIVE (always-on). The ruleset below applies to every response. "stop meini style" or "modo normal" turns it off for this session; delete %s to turn always-on off for good.\n\n%s\n' \
  "$flag_path" "$body"
