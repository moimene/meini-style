...
def validate_package_manifest() -> None:
    ...
    assert package.get("pi") == {
        "extensions": ["./extensions/meini-style.ts"],
        "skills": ["./skills"],
    }
...
def status_texts(events: list[dict[str, Any]]) -> list[str | None]:
    return [
        ...
        and event.get("statusKey") == "meini-style"
    ]
...
def latest_enabled(entries_response: dict[str, Any]) -> bool:
    states = [
        ...
        and entry.get("customType") == "meini-style-state"
    ]
    if not states or not isinstance(states[-1], bool):
        raise AssertionError("No persisted meini-style state found")
    return states[-1]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke-test the meini-style extension without a model request."
    )
    parser.add_argument(
        "--runtime",
        choices=("pi", "omp"),
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"meini-style-{args.runtime}-"
    ) as agent_dir:
...
"""
        ...
        if args.runtime == "omp":
            try:
                ...
                assert "meini-style" in command_names
                assert "skill:meini-style" in command_names
                assert any(
                    "MEINI ON" in (text or "")
        ...
        try:
            ...
            assert "meini-style" in command_names
            assert "skill:meini-style" in command_names
            assert any("MEINI ON" in (text or "") for text in status_texts(startup_events))

            entries, _ = client.request("entries-startup", {"type": "get_entries"})
            assert message_count(entries, "meini-style-rules") == 1
            assert message_count(entries, "meini-style-disabled") == 0
            ...
            assert any("MEINI ON" in (text or "") for text in status_texts(reload_events))
            ...
            toggled_off, toggle_off_events = client.request(
                "toggle-off",
                {"type": "prompt", "message": "/meini-style"},
            )
            ...
            assert message_count(entries, "meini-style-disabled") == 1
            ...
            assert reloaded["success"] is True
            assert not any("MEINI ON" in (text or "") for text in status_texts(reload_events))
            ...
            assert latest_enabled(entries) is True
            assert message_count(entries, "meini-style-rules") == 3
        ...
        if args.runtime == "pi":
            Path(agent_dir, ".meini-style-always").touch()
            always_on = RpcClient(
                executable,
                env,
                "--no-session",
