// meini-style — OpenCode plugin.
//
// Mirrors the Claude Code / Codex behaviour for OpenCode: the skill in
// `skills/meini-style/SKILL.md` is the single source of truth for the ruleset.
//
//   • On demand   — registers the skills directory and a `/meini-style`
...
//
// Opt in to always-on:   touch ~/.config/opencode/.meini-style-always
// Opt back out:          rm ~/.config/opencode/.meini-style-always
//
// Install — add to opencode.json:
//   { "plugin": ["./.opencode/plugins/meini-style.mjs"] }
...
const skillPath = path.join(skillsDir, 'meini-style', 'SKILL.md');
const commandPath = path.join(__dirname, '..', 'command', 'meini-style.md');
...
// Always-on opt-in flag, mirroring Claude Code's ~/.claude/.meini-style-always
...
export default async () => {
  return {
    // Make the skill discoverable (so the `skill` tool and the /meini-style
    // command can load it).
    config: async (config) => {
      ...
      try {
        ...
        if (!config.command['meini-style']) {
          config.command['meini-style'] = await commandDefinition();
        }
      } catch (e) {
    ...
    // flag file exists. "stop meini style" turns it off for the session (the
    ...
    'experimental.chat.system.transform': async (_input, output) => {
      ...
      const header =
        'MEINI STYLE ACTIVE (always-on). The ruleset below applies to every ' +
        'response. "stop meini style", "modo normal" or "normal mode" turns it off for this ' +
